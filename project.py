...
def DisplayForm():
    ...
    global tree
    global SEARCH
    global fname,lname,gender,email,address,contact
    SEARCH = StringVar()
    fname = StringVar()
    lname = StringVar()
    email = StringVar()
    ...
    lbl_txtsearch = Label(LeftViewForm, text="Enter fname to Search", font=('verdana', 10),bg="#0B4670",fg="white")
    lbl_txtsearch.pack()
    ...
    tree = ttk.Treeview(MidViewForm,columns=("Student Id", "Name", "Contact","Gender", "Email","Rollno","Branch"),
...
def Update():
    ...
    if fname1=='' or lname1==''or gender1=='' or email1=='' or address1==''or contact1=='':
        ...
        #update query
        conn.execute('UPDATE REGISTRATION SET FNAME=?,LNAME=?,GENDER=?,EMAIL=?,ADDRESS=?,CONTACT=? WHERE RID = ?',(fname1,lname1,gender1,email1,address1,contact1, selecteditem[0]))
        conn.commit()
...
def register():
    ...
    if fname1=='' or lname1==''or gender1=='' or email1=='' or address1==''or contact1=='':
        ...
        #execute query
        conn.execute('INSERT INTO REGISTRATION (FNAME,LNAME,GENDER,EMAIL,ADDRESS,CONTACT) \
              VALUES (?,?,?,?,?,?)',(fname1,lname1,gender1,email1,address1,contact1));
        conn.commit()
